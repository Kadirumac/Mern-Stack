...
import {Breadcrumb,Modal,ModalBody, BreadcrumbItem, Card, CardBody, CardHeader, Media} from 'reactstrap';
    ...
    render () {
      ...
      return (
        ...
        <ModalBody>
          ...
          <ModalVideo isOpen={this.state.isOpen} onClose={() => this.setState({isOpen: false})} channel='youtube'   videoId='pRpeEdMmmQ0'  />
          </ModalBody>
        </Modal>
        <button className="btn-info" outline onClick={this.openModal}>Company<br/>Video</button>
        </React.Fragment>
      )
    }
  }
const RenderLeader = ({leaders}) => {
    return leaders != null ? leaders.leaders.map((leader, k) =>
            (     <FadeTransform in 
                transformProps={{
